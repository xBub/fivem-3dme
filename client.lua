...
local function Display(ped, text)
    ...
    if dist <= Config.visual.dist then
        ...
        -- Display
        local offset = 0.5 + pedDisplaying[ped] * 0.1
        while display do
...
RegisterNetEvent('xBub_:shareDisplay')
AddEventHandler('xBub_:shareDisplay', function(text, serverId)
...
AddEventHandler('xBub_:sendProximityMessageMe', function(id, name, message)
  ...
  local plid = GetPlayerServerId()
  if pid == myId then
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^5[ME] ^3" .. name .. "^6(#" .. id .. "):^7 " .. message)
